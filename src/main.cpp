...
int main(int, char **)
{
    ...
    // io.Fonts->GetGlyphRangesChineseFull()); IM_ASSERT(font != NULL);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            static bool check_switch = false;
            ...
            ImGui::Checkbox("Switch", &check_switch);
            if (!check_switch)
            {
                auto binsps = hexToChar(test);
                bzero(mem_data.data() + binsps.size(), mem_data.size() - binsps.size());
                memcpy(mem_data.data(), binsps.data(), binsps.size());
                mem_edit_1.DrawWindow("Memory Editor", mem_data.data(), mem_data.size());

                static bool b_parser_sps = false;

                if (ImGui::Button("Parser SPS")) // Buttons return true when clicked (most widgets return true when
                                                 // edited/activated)
                {
            ...
            {
                mem_edit_1.DrawWindow("Memory Editor", test.data(), test.size());
            }
